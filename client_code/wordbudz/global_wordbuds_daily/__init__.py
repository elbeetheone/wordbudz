from ._anvil_designer import global_wordbuds_dailyTemplate
from anvil import *
import anvil.server
import time
...
class global_wordbuds_daily(global_wordbuds_dailyTemplate):
  def __init__(self, **properties):
    ...
    self.stage = str(date.today())

    if self.stage != anvil.server.call_s("check_playtime", self.link_1.text, 'daily'):
      #if today's date is not the last played date, upload new game
      self.user_words = anvil.server.call_s("get_it", 'daily')[0]
    ...
    if self.stage == anvil.server.call_s("check_playtime", self.link_1.text, 'daily'):
      ...
      self.repeating_panel.items = anvil.server.call_s("check_words", self.link_1.text, 'daily')
  ...
  def record_click(self, **event_args):
    ...
    if len(self.user_words) != 0:
      ...
      url = f"""https://speakeasi.streamlit.app/?embedded=true&bar=budz&route=daily&user={self.link_1.text}&foo={self.foo}&user_words={the_words}"""
...
Download & Play @ https://rb.gy/dfzb5p/#word'''
  ...
  def timer_2_tick(self, **event_args):
    ...
    try:
      if self.stage == anvil.server.call("check_playtime", self.link_1.text, 'daily'):
        self.timer_2.interval = 0
        anvil.server.call('next_stage', self.link_1.text, [{"Avg_rating": 0,"Played_time": 0}], 'daily')
        open_form("global_wordbuds_daily")
        # else:
        #   open_form('gameplay')
